...
class NewBlocksServer(Server):
    ...
    def handle_client_connection(self, client_sock):
        ...
        logging.info(f"Thread {id}: Received connection. Type {msg}")
    ...
    def _handle_block_uploader(self, client_sock):
        ...
        while True:
            try:
                ...
                logging.info(f"Thread {id}: Message {msg}")
                new_block = Block.deserialize(msg)
                logging.info(f"Thread {id}: Message received {client_sock.getpeername()}. Block Info: {new_block}")
            except OSError:
                logging.info(f"Thread {id}: Error while reading socket {client_sock}")
            # finally:
            #     client_sock.close()
    ...
    def _announce_new_block(self):
        # id = threading.get_ident()
        # with open(f"/blockchain-node/{id}.txt", "w") as file:
