...
class ChunkManager:
    ...
    def add_to_chunk_queue(self, chunk):
        ...
        if added:
            logging.info(f"THREAD {None}: Chunk {chunk} succesfully added to queue.")
            self._wait_or_force_dispatch()
        else:
            logging.error(f"THREAD {None}: Chunk queue already full! Discarding chunk: {chunk}")

        return added

    def _wait_or_force_dispatch(self):
        if self.pending_chunks_queue.qsize() >= MAX_CHUNKS_PER_BLOCK:
            logging.info(f"THREAD {None}: Reached max chunks per block, forcing dispatch")
